...
import type { StopwatchQuestionModel } from "./models.ts";
import { nanoid } from "./nanoid";

interface QuestionStoreState
  extends Omit<StopwatchQuestionModel, "startAt" | "endedAt"> {
  isPaused: boolean;
  isRunning: boolean;
...
const generateDefaultTimer = (): QuestionStoreState => ({
  isRunning: false,
  isPaused: true,
  startAt: null,
  pausedAt: null,
...
export const useQuestionStore = create<
  QuestionStoreState & QuestionStoreActions
>((set) => {
  return {
    isRunning: false,
    isPaused: true,
